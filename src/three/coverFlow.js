...
export class CoverFlow {
  ...
  renderPositions() {
    this.meshes.forEach((mesh, index) => {
      ...
      const scale = distance < 0.5 ? 1.08 : Math.max(0.68, 1 - distance * 0.18);
      const x = offset * 1.72;
      ...
      mesh.renderOrder = Math.max(1, Math.round(1000 - distance * 100));
  ...
  snapTo(index, duration = 0.62) {
    ...
    gsap.to(this, { targetPosition: this.index, duration, ease: 'power3.out', onUpdate: () => this.renderPositions() });
  }

  step(direction) { this.snapTo(this.index + direction); }

  tick() {
    const difference = this.targetPosition - this.position;
    if (Math.abs(difference) > 0.0001 || Math.abs(this.velocity) > 0.0005) {
      this.position += difference * 0.18 + this.velocity;
      this.velocity *= 0.86;
      this.position = Math.max(0, Math.min(this.items.length - 1, this.position));
      this.renderPositions();
    }
  }
